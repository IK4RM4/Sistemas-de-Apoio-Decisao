# 02_clean_data.R - versão corrigida para o erro snow_3h
...
# Inspecionar os nomes das colunas para debug
print("Colunas disponíveis em weather_raw:")
print(colnames(weather_raw))

# 2. Limpar dados de partilha de bicicletas
bike_clean <- bike_raw %>%
  clean_names() %>%
  # Usar mutate vectorizado corretamente
  mutate(across(where(is.character), str_squish)) %>%
  mutate(across(where(is.character), ~str_remove_all(., "<.*?>"))) %>%
  # Extrair números de bicicletas quando possível
  mutate(
    bicycles = parse_number(str_extract(system, "\\d[\\d,]*\\s*(bicycles|bikes)")),
    stations = parse_number(str_extract(system, "\\d[\\d,]*\\s*(stations)")),
    system_clean = str_remove_all(system, "\\[\\d+\\]") # Remover referências wiki
  )
...
# Verificar primeiro se as colunas existem antes de usá-las
weather_clean <- weather_raw %>%
  ...
  mutate(
    ...
    date = as_date(dt_txt),
    hour = hour(dt_txt),
    # Valores para o modelo de previsão
    ...
    rainfall_mm = if("rain_3h" %in% names(.)) {
      ifelse(is.na(rain_3h), 0, rain_3h)
    } else {
      0
    },
    snowfall_cm = if("snow_3h" %in% names(.)) {
      ifelse(is.na(snow_3h), 0, snow_3h)
    } else {
      0
    }
  )
